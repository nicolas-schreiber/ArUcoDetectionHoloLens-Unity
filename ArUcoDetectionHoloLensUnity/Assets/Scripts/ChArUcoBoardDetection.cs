...
namespace ArUcoDetectionHoloLensUnity
{
    ...
    public class ChArUcoBoardDetection : MonoBehaviour
    {
        public Text myText;
        ...
        // Marker size in meters: 0.035 m
        public float markerSize;

        // Params for Charuco detection
        // Square size in meters: 0.022 m
        public float squareSize;

        // Number of elements in X direction on ChArUco
        public int squaresX;
        
        // Number of elements in Y direction on ChArUco
        public int squaresY;
        ...
        async Task StartHoloLensMediaFrameSourceGroups()
        {
            ...
            // PV
            Debug.Log("HoloLensForCVUnity.ChArUcoDetection.StartHoloLensMediaFrameSourceGroup: Setting up sensor frame streamer");
            ...
            // Spatial perception
            Debug.Log("HoloLensForCVUnity.ChArUcoDetection.StartHoloLensMediaFrameSourceGroup: Setting up spatial perception");
            _spatialPerception = new SpatialPerception();

            // Enable media frame source groups
            // PV
            Debug.Log("HoloLensForCVUnity.ChArUcoDetection.StartHoloLensMediaFrameSourceGroup: Setting up the media frame source group");
            ...
            // Photo video
            Debug.Log("HoloLensForCVUnity.ChArUcoDetection.StartHoloLensMediaFrameSourceGroup: Starting the media frame source group");
            ...
            {
                Debug.Log("ChArUcoDetectionHoloLensUnity.ChArUcoBoardDetection: Could not get pointer to Unity spatial coordinate system.");
                throw;
            }

            // Initialize the aruco marker detector with parameters
            await _pvMediaFrameSourceGroup.StartChArUcoBoardTrackerAsync(
                squaresX,
                squaresY,
                squareSize,
		        markerSize,
        ...
        unsafe void UpdateHoloLensMediaFrameSourceGroup()
        {
            ...
            DetectedChArUcoBoard detectedChArUcoBoard = _pvMediaFrameSourceGroup.DetectChArUcoBoard(_sensorType);

            //detectedArUcoMarkers = 
            //    _arUcoMarkerTracker.DetectArUcoMarkersInFrame(latestPvCameraFrame);

            // If we detect a marker, display
            if (detectedChArUcoBoard.DetectedArUcoMarkers.Count != 0)
            {
                // Get pose from OpenCV and format for Unity
                Vector3 position = CvUtils.Vec3FromFloat3(detectedChArUcoBoard.Position);
                position.y *= -1f;
                Quaternion rotation = CvUtils.RotationQuatFromRodrigues(CvUtils.Vec3FromFloat3(detectedChArUcoBoard.Rotation));
                Matrix4x4 cameraToWorldUnity = CvUtils.Mat4x4FromFloat4x4(detectedChArUcoBoard.CameraToWorldUnity);
